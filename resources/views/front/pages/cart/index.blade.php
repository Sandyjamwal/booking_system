...
<div class="container pb-5 mb-2 cart-checkoutpage">
  ...
  <div class="cart-checkoutpage1">
    ...
    $subtotal = $value->attributes->ExtraDiscount/100 * $total; 
    @endphp
    <!-- Cart Item-->
    <div class="row">
    <div class="cart-item-extra">
      <div class="cart-item d-md-flex justify-content-between pb-0 mb-0">
        ...
        <div class="px-3 my-3 text-center">
          <div class="cart-item-label">WithoutPickup</div>
          <div class="count-input">
            <option>{{ ($value->attributes->ExtraDiscount == 10) ? $value->attributes->ExtraDiscount.'%OFF' : 'N/A'}}</option>
          </div>
        </div>
        <div class="px-3 my-3 text-center"> 
          <div class="cart-item-label">Total</div><span class="text-xl font-weight-medium">{{ ($value->price * $value->quantity - $extra10Per)}} AED</span>
        </div>
        <div class="px-3 my-3 text-center">
    ...
    <div class="subt-disct-extra">
            <div class="py-2 alltotal">
                <span class="d-inline-block align-middle text-sm text-muted font-weight-medium text-uppercase mr-2">Subtotal:</span><span class="d-inline-block align-middle text-xl font-weight-medium subtotal-value final-value" id="basket-subtotal">{{ number_format($total-$subtotal, 2) }} AED</span>
            </div>
            <div class="py-2 alltotal">
            ...
            <div class="py-2 alltotal">
                <span class="d-inline-block align-middle text-sm text-muted font-weight-medium text-uppercase mr-2">Total:</span><span class="d-inline-block align-middle text-xl font-weight-medium subtotal-value final-value grandCoupon grandTotal" id="grand-total">{{ number_format($total + $extraAmount - $discount - $subtotal, 2) }} AED</span>
                <input type="hidden" id="grandTotal" value="{{ $total + $extraAmount - $discount - $subtotal }}">
            </div>
        </div>
        </div>
</div>
